...
namespace SpawnSheet
{
	...
	internal static class PaintToolsEx
	{
		...
		internal static string importPath = Path.Combine(Main.SavePath, "SpawnSheet_PaintTools.txt");
		internal static string exportPath = Path.Combine(Main.SavePath);

		private static Uri schematicsurl = new Uri("http://javid.ddns.net/tModLoader/jopojellymods/SpawnSheet_Schematics_GetList.php");
		...
		internal static void GetSchematicsComplete(object sender, UploadValuesCompletedEventArgs e) {
			if (!e.Cancelled) {
				...
				if (schematicslist != null) {
					...
					SpawnSheet.instance.numberOnlineToLoad = SpawnSheet.DefaultNumberOnlineToLoad;
					...
					//if (list.Count > 0)
					//	SpawnSheet.instance.paintToolsUI.view.Add(list.ToArray());
				}
			}
			else {
		...
		internal static void LoadSingleSchematic() {
			...
			try {
				...
				if (tiles.GetLength(0) > 0) {
					...
					paintToolsSlot.rating = rating;
					paintToolsSlot.vote = vote;
					SpawnSheet.instance.paintToolsUI.view.AddEndDontSelect(paintToolsSlot);
				}
			}
			catch { }

			SpawnSheet.instance.numberOnlineToLoad--;
		...
		internal static void OnlineImport(PaintToolsView paintToolsView) {
			...
			if (SpawnSheet.instance.paintToolsUI.view.slotList.Any(x => x.browserID > 0)) {
		...
		internal static void LoadNextX(PaintToolsView view) {
			...
			SpawnSheet.instance.paintToolsUI.view.RemoveAllOnline();
			SpawnSheet.instance.numberOnlineToLoad = SpawnSheet.DefaultNumberOnlineToLoad;
		}
		...
		public static void Export(PaintToolsView paintToolsView) {
			try {
				...
				foreach (var slot in paintToolsView.slotList) {
					path = Path.Combine(exportPath, $"SpawnSheet_PaintTools_{index++}.json");
					list.Add(path);
