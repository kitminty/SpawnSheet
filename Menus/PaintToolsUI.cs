...
namespace SpawnSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "PaintTools") => SpawnSheet.CSText(category, key);
		internal static string CSTextSB(string key, string category = "SchematicsBrowser") => SpawnSheet.CSText(category, key);
		...
		public PaintToolsUI(SpawnSheet mod) {
			categories.Clear();
			...
			uIImage.Tooltip = $"Load Next {SpawnSheet.DefaultNumberOnlineToLoad}";
			this.AddChild(uIImage);
			*/
			...
			upVoteButton = new UIImage(SpawnSheet.instance.Assets.Request<Texture2D>("UI/VoteUp", ReLogic.Content.AssetRequestMode.ImmediateLoad));
			...
			downVoteButton = new UIImage(SpawnSheet.instance.Assets.Request<Texture2D>("UI/VoteDown", ReLogic.Content.AssetRequestMode.ImmediateLoad));
		...
		private static Uri submiturl = new Uri("http://javid.ddns.net/tModLoader/jopojellymods/SpawnSheet_Schematics_Submit.php");
		...
		private void Submit() {
			...
			try {
				using (WebClient client = new WebClient()) {
					...
					*/
					...
					if (string.IsNullOrEmpty(base64tiles)) {
						...
						{
							{ "version", SpawnSheet.instance.Version.ToString() },
							{ "steamid64", steamid64 },
				...
				Main.NewText(Language.GetTextValue($"Mods.SpawnSheet.SchematicsBrowser.SchematicsServerProblemX", 5));
				submitWait = false;
			}
		}
			...
			try {
				if (!e.Cancelled) {
					...
					if (jsonObject["message"] != null) {
						...
						Main.NewText(Language.GetTextValue($"Mods.SpawnSheet.SchematicsBrowser.SchematicsServerProblemX", 12));
					}
				}
				else {
					Main.NewText(Language.GetTextValue($"Mods.SpawnSheet.SchematicsBrowser.SchematicsServerProblemX", 2));
				}
			}
			catch {
				Main.NewText(Language.GetTextValue($"Mods.SpawnSheet.SchematicsBrowser.SchematicsServerProblemX", 3));
			}
			submitWait = false;
		}

		private static Uri voteurl = new Uri("http://javid.ddns.net/tModLoader/jopojellymods/SpawnSheet_Schematics_Vote.php");
		...
		private void Vote(bool up) {
			...
			SpawnSheet.instance.paintToolsUI.infoMessage.Text = PaintToolsSlot.CurrentSelect.browserName + ": " + PaintToolsSlot.CurrentSelect.rating;
			PaintToolsSlot.CurrentSelect.vote = voteInt;
			SpawnSheet.instance.paintToolsUI.upVoteButton.ForegroundColor = Color.White;
			SpawnSheet.instance.paintToolsUI.downVoteButton.ForegroundColor = Color.White;
			if (PaintToolsSlot.CurrentSelect.vote == 1)
				SpawnSheet.instance.paintToolsUI.upVoteButton.ForegroundColor = Color.Gray;
			if (PaintToolsSlot.CurrentSelect.vote == -1)
				SpawnSheet.instance.paintToolsUI.downVoteButton.ForegroundColor = Color.Gray;
			if (SocialAPI.Mode != SocialMode.Steam) {
				Main.NewText(CSTextSB("OnlineSchematicsOnlyWorksOnSteamVersion"));
				return;
			}
