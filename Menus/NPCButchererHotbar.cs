...
namespace SpawnSheet.Menus
{
	internal class NPCButchererHotbar : UIHotbar
	{
		...
		internal static string CSText(string key, string category = "Butcherer") => SpawnSheet.CSText(category, key);
		...
		public NPCButchererHotbar(SpawnSheet mod) {
			this.mod = mod;
		...
		private static void SyncButcher(int butcherType = 0) {
			...
			netMessage.Write((byte)SpawnSheetMessageType.ButcherNPCs);
