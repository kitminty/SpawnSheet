...
namespace SpawnSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "ExtendedCheatMenu") => SpawnSheet.CSText(category, key);
		public SpawnSheet mod;
		private static UIImage[] buttons = new UIImage[SpawnSheet.ButtonTexture.Count];
		private float spacing = 16f;

		public ExtendedCheatMenu(SpawnSheet mod) {
			buttons = new UIImage[SpawnSheet.ButtonTexture.Count];
			this.mod = mod;
			this.CanMove = true;
			base.Width = spacing * 2;
			base.Height = spacing * 2;

			if (SpawnSheet.ButtonTexture.Count == 0) {
			...
			if (SpawnSheet.ButtonTexture.Count > 0) {
				int count = SpawnSheet.ButtonTexture.Count;

				int cols = (count + 4) / 5;
				int rows = count >= 5 ? 5 : count;

				for (int j = 0; j < SpawnSheet.ButtonTexture.Count; j++) {
					Asset<Texture2D> textureAsset = SpawnSheet.ButtonTexture[j];
