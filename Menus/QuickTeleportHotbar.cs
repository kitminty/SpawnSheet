...
namespace SpawnSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "Teleport") => SpawnSheet.CSText(category, key);
		...
		public QuickTeleportHotbar(SpawnSheet mod) {
			this.mod = mod;
		...
		private static void SyncTeleport(int teleportType = 0) {
			...
			netMessage.Write((byte)SpawnSheetMessageType.TeleportPlayer);
