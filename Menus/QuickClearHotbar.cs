...
namespace SpawnSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "QuickClear") => SpawnSheet.CSText(category, key);
		...
		public QuickClearHotbar(SpawnSheet mod) {
			this.mod = mod;
		...
		private static void SyncQuickClear(int clearType = 0) {
			...
			netMessage.Write((byte)SpawnSheetMessageType.QuickClear);
