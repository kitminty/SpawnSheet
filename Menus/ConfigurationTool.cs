...
namespace SpawnSheet.Menus
{
	internal class ConfigurationTool
	{
		...
		public static SpawnSheet SpawnSheet;

		public static UIImage GetButton(Mod mod) {
			SpawnSheet = mod as SpawnSheet;

			configurationWindow = new ConfigurationWindow(SpawnSheet);
		...
		private static void bConfigurationToggle_onLeftClick(object sender, EventArgs e) {
			...
			if (ConfigurationTool.configurationWindow.selected) {
				...
				SpawnSheet.hotbar.DisableAllWindows();
				configurationWindow.selected = true;
	...
	{
		internal static string CSText(string key, string category = "ConfigurationTool") => SpawnSheet.CSText(category, key);
		public SpawnSheet mod;
		private float spacing = 16f;

		public ConfigurationWindow(SpawnSheet mod) {
			this.mod = mod;
			this.CanMove = true;
			base.Width = 280;
			base.Height = 358;
			...
			for (int i = 0; i < labels.Length; i++) {
				...
				cb.SelectedChanged += (a, b) => {
					...
					((SpawnSheet)mod).hotbar.ChangedConfiguration();
