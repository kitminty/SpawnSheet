...
namespace SpawnSheet.Menus
{
	internal class PaintToolsHotbar : UIHotbar
	{
		...
		public PaintToolsHotbar(SpawnSheet mod) {
			this.mod = mod;
		...
		public override void Draw(SpriteBatch spriteBatch) {
			if (Visible) {
				...
				scissorRectangle = SpawnSheet.GetClippingRectangle(spriteBatch, scissorRectangle);
			...
			try {
				if (Visible && !base.IsMouseInside() && (StampToolActive || EyeDropperActive)) {
					if (!Main.LocalPlayer.mouseInterface) {
						...
						Terraria.GameInput.PlayerInput.LockVanillaMouseScroll("SpawnSheet/DrawBrush");

						if (Terraria.GameInput.PlayerInput.ScrollWheelDeltaForUI != 0) {
							if (Terraria.GameInput.PlayerInput.ScrollWheelDeltaForUI < 0)
								SpawnSheet.instance.paintToolsUI.btnSnap.NextIamge();
							else
								SpawnSheet.instance.paintToolsUI.btnSnap.PrevIamge();
						}
					}
				}
			}
		...
		private void DrawBrush() {
			if (EyeDropperActive) {
				...
				Vector2 vector = Snap.GetSnapPosition(SpawnSheet.instance.paintToolsUI.SnapType, width, height, constrainToAxis, constrainedX, constrainedY, false);
			...
			if (selected && (EyeDropperActive || StampToolActive)) {
				...
				if (EyeDropperActive) {
					...
					if (justLeftMouseDown) {
						if (startTileX != -1 && startTileY != -1 && lastMouseTileX != -1 && lastMouseTileY != -1) {
							...
							SpawnSheet.instance.paintToolsUI.AddSlot(PaintToolsEx.GetStampInfo(StampTiles));
				...
				if (StampToolActive) {
					...
					if (leftMouseDown && stampInfo != null) {
						...
						Point point = Snap.GetSnapPosition(SpawnSheet.instance.paintToolsUI.SnapType, width, height, constrainToAxis, constrainedX, constrainedY, true).ToPoint();
