...
namespace SpawnSheet.Menus
{
	internal class Hotbar : UIWindow
	{
		...
		public Hotbar(SpawnSheet mod) {
			...
			this.button.Position = new Vector2(10f, -this.button.Height);
			...
			this.bCycleExtraAccessorySlots.onLeftClick += (s, e) => {
				SpawnSheetPlayer SpawnSheetPlayer = Main.LocalPlayer.GetModPlayer<SpawnSheetPlayer>();
				SpawnSheetPlayer.numberExtraAccessoriesEnabled = (SpawnSheetPlayer.numberExtraAccessoriesEnabled + 1) % (SpawnSheetPlayer.MaxExtraAccessories + 1);
				bCycleExtraAccessorySlots.Tooltip = CSText("ExtraAccessorySlots") + ": " + SpawnSheetPlayer.numberExtraAccessoriesEnabled;
			};
			this.bCycleExtraAccessorySlots.onRightClick += (s, e) => {
				SpawnSheetPlayer SpawnSheetPlayer = Main.LocalPlayer.GetModPlayer<SpawnSheetPlayer>();
				SpawnSheetPlayer.numberExtraAccessoriesEnabled = SpawnSheetPlayer.numberExtraAccessoriesEnabled == 0 ? 0 : (SpawnSheetPlayer.numberExtraAccessoriesEnabled - 1) % (SpawnSheetPlayer.MaxExtraAccessories + 1);
				bCycleExtraAccessorySlots.Tooltip = CSText("ExtraAccessorySlots") + ": " + SpawnSheetPlayer.numberExtraAccessoriesEnabled;
			};
		...
		private static void SyncVacuum() {
			...
			netMessage.Write((byte)SpawnSheetMessageType.VacuumItems);
			netMessage.Send();
		}
		...
		public override void Update() {
			try {
				...
				if (mod.paintToolsHotbar.Visible || mod.quickTeleportHotbar.Visible || mod.quickClearHotbar.Visible || mod.npcButchererHotbar.Visible/* || mod.eventManagerHotbar.Visible*/) {
					...
					this.button.Position = new Vector2(10f, -this.button.Height - Math.Max(0, (base.Position.Y - currentHotbar.Position.Y)));
					this.arrow.Position = new Vector2(10f, -this.button.Height - Math.Max(0, (base.Position.Y - currentHotbar.Position.Y)));
					button.CenterXAxisToParentCenter(offset);
					arrow.CenterXAxisToParentCenter(offset);
				}
				else {
					this.button.Position = new Vector2(10f, -this.button.Height);
					if (Main.LocalPlayer.name == SpawnSheetServerConfig.Instance.Owner) {
						button.CenterXAxisToParentCenter();
						arrow.CenterXAxisToParentCenter();
					} else {
						button.CenterXAxisToParentCenter((int)39.5);
						arrow.CenterXAxisToParentCenter((int)39.5);
					}
				}
				base.CenterXAxisToParentCenter();
				base.Update();
		...
		public void ChangedConfiguration() {
			...
			MinionSlotBooster.button.Visible = ConfigurationLoader.personalConfiguration.MinionBooster && SpawnSheet.instance.herosPermissions[SpawnSheet.MinionBooster_Permission];
			bToggleClearMenu.Visible = ConfigurationLoader.personalConfiguration.ClearMenu && SpawnSheet.instance.herosPermissions[SpawnSheet.ClearItemNPCProjectile_Permission];
			bTogglePaintTools.Visible = ConfigurationLoader.personalConfiguration.PaintTools && SpawnSheet.instance.herosPermissions[SpawnSheet.PaintTools_Permission];
			bToggleExtendedCheat.Visible = ConfigurationLoader.personalConfiguration.ModExtensions && SpawnSheet.instance.herosPermissions[SpawnSheet.SpawnSheetExtensions_Permission];
			bCycleExtraAccessorySlots.Visible = ConfigurationLoader.personalConfiguration.ExtraAccessorySlots && SpawnSheet.instance.herosPermissions[SpawnSheet.ExtraAccessories_Permission];
			bVacuum.Visible = ConfigurationLoader.personalConfiguration.Vacuum && SpawnSheet.instance.herosPermissions[SpawnSheet.Vacuum_Permission];
			bToggleNPCButcherer.Visible = ConfigurationLoader.personalConfiguration.Butcher && SpawnSheet.instance.herosPermissions[SpawnSheet.NPCButcher_Permission];
			bToggleQuickTeleport.Visible = ConfigurationLoader.personalConfiguration.Waypoints && SpawnSheet.instance.herosPermissions[SpawnSheet.QuickTeleport_Permission];
		...
		public void ResizeSmall() {
			...
			for (int i = 0; i < this.buttonView.children.Count; i++) {
				if (this.buttonView.children[i].Visible) {
					this.buttonView.children[i].X = num+10;
					num += this.buttonView.children[i].Width + this.spacing;
				}
			}
			//base.Width = 127f;
			//base.Position = new Vector2(100);
			base.Width = num+20;
			this.buttonView.Width = base.Width;
			this.button.CenterXAxisToParentCenter();
			this.button.Position = new Vector2(23.5f, -this.button.Height);
			this.arrow.Position = new Vector2(23.5f, -this.button.Height);
		}

		public void Hide() {
			this.hidden = true;
			if (Main.LocalPlayer.name == SpawnSheetServerConfig.Instance.Owner) {
				bToggleItemBrowser.Visible = true; 
				bToggleNPCBrowser.Visible = true;
				bToggleRecipeBrowser.Visible = true;
				MinionSlotBooster.button.Visible = true;
				bToggleClearMenu.Visible = true;
				bTogglePaintTools.Visible = true;
				bToggleExtendedCheat.Visible = true;
				bCycleExtraAccessorySlots.Visible = true;
				bVacuum.Visible = true;
				bToggleNPCButcherer.Visible = true;
				bToggleQuickTeleport.Visible = true;
				LightHack.button.Visible = true;
				GodMode.button.Visible = true;
				ConfigurationTool.button.Visible = true;
				SpawnRateMultiplier.button.Visible = true;
				Resize();
			} else {
				bToggleItemBrowser.Visible = false;
				bToggleNPCBrowser.Visible = false;
				bToggleRecipeBrowser.Visible = false;
				MinionSlotBooster.button.Visible = false;
				bToggleClearMenu.Visible = false;
				bTogglePaintTools.Visible = false;
				bToggleExtendedCheat.Visible = false;
				bCycleExtraAccessorySlots.Visible = false;
				bVacuum.Visible = false;
				bToggleNPCButcherer.Visible = false;
				bToggleQuickTeleport.Visible = false;
				LightHack.button.Visible = false;
				GodMode.button.Visible = false;
				ConfigurationTool.button.Visible = false;
				SpawnRateMultiplier.button.Visible = true;
				ResizeSmall();
			}
