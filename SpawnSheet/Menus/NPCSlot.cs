...
namespace SpawnSheet.Menus
{
	internal class NPCSlot : UIView
	{
		...
		private void Init(int npcNum, int index) {
			...
			base.onRightClick += (s, e) => {
				if (Main.netMode == 1) {
					...
					var message = SpawnSheet.instance.GetPacket();
					message.Write((byte)SpawnSheetMessageType.RequestFilterNPC);
					message.Write(netID);
		...
		//	netMessage.Write((byte)SpawnSheetMessageType.RequestToggleNPCSpawn);
		...
		//{
		//	int useValue = netID;
		//	NPCSlot slot = ((SpawnSheet)SpawnSheet.instance).npcBrowser.npcView.allNPCSlot[whoAmI];
		...
		//{
		//	var netMessage = SpawnSheet.instance.GetPacket();
		//	netMessage.Write((byte)SpawnSheetMessageType.FilterNPC);
		//	netMessage.Write(whoAmI);
		//	netMessage.Send();
		//}
		...
		private static void SyncNPC(int type, int syncID = 0) {
			var netMessage = SpawnSheet.instance.GetPacket();
			netMessage.Write((byte)SpawnSheetMessageType.SpawnNPC);
