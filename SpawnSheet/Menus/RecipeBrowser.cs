...
namespace SpawnSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "RecipeBrowser") => SpawnSheet.CSText(category, key);
		...
		public RecipeBrowserWindow(SpawnSheet mod) {
			categories.Clear();
		...
		public override void Update() {
			...
			if (selectedRecipeChanged) {
				//ErrorLogger.Log("1");
				//foreach(var a in SpawnSheet.ButtonClicked)
				//{
				//	Main.NewText(">");
