...
namespace SpawnSheet
{
	internal class SpawnSheet : Mod
	{
		...
		internal const string SpawnSheetExtensions_Permission = "SpawnSheetExtensions";
		internal const string SpawnSheetExtensions_Display = "Cheat Sheet Extensions";
		...
		public override void Load() {
			...
			herosPermissions[SpawnSheetExtensions_Permission] = true;
		...
		public override void Unload() {
			...
			ConfigurationTool.SpawnSheet = null;
		...
		internal static string CSText(string category, string key) {
			...
			//return translations[$"Mods.SpawnSheet.{category}.{key}"].GetTranslation(Language.ActiveCulture);
		...
		*/
		...
		private void SetupHEROsModIntegration(Mod herosMod) {
			...
			var permissions = new List<ValueTuple<string, string>>() {
				...
				(SpawnSheetExtensions_Permission,SpawnSheetExtensions_Display),
		...
		public static bool IsPlayerLocalServerOwner(Player player) {
			if (SpawnSheetServerConfig.Instance.Owner == player.name) {
				return true;
			}
		...
		public override void HandlePacket(BinaryReader reader, int whoAmI) {
			...
			switch (msgType) {
				...
				case SpawnSheetMessageType.InformFilterNPC:
					...
					break;
				//case SpawnSheetMessageType.RequestToggleNPCSpawn:
	...
	public static class SpawnSheetInterface
	{
		...
		public static IEnumerable<Item> GetExtraAccessories(Player player) {
			return player.GetModPlayer<SpawnSheetPlayer>().ExtraAccessories;
		}
		...
		public static IEnumerable<Item> GetEnabledExtraAccessories(Player player) {
			var SpawnSheetPlayer = player.GetModPlayer<SpawnSheetPlayer>();
			return SpawnSheetPlayer.ExtraAccessories.Take(SpawnSheetPlayer.numberExtraAccessoriesEnabled);
		}
		...
		public static Item GetExtraAccessory(Player player, int index) {
			if (index < 0 || index >= SpawnSheetPlayer.MaxExtraAccessories)
				return null;
			return player.GetModPlayer<SpawnSheetPlayer>().ExtraAccessories[index];
		}
	}
